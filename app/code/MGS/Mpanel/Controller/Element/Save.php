		...
		if($this->customerSession->getUsePanel() == 1){
			...
			switch ($data['type']) {
				...
				case "recent_view_products":
                    $dataInit = ['numbercol, perrow'];
					$data = $this->reInitData($data, $dataInit);
                    
					if($data['setting']['template']=='list.phtml'){
                        $content = '<div class="row-'.$data['setting']['numbercol'].'">';
                        $content .= '{{widget type="Magento\Catalog\Block\Widget\RecentlyViewed" uiComponent="widget_recently_viewed" page_size="'.$data['setting']['limit'].'" show_attributes="name,image,price" show_buttons="add_to_cart,add_to_compare,add_to_wishlist" template="product/widget/viewed/list.phtml"}}</div>';
					}
					
					if($data['setting']['template']=='grid.phtml'){
                         $content = '<div class="row-'.$data['setting']['perrow'].'">';
                         $content .= '{{widget type="Magento\Catalog\Block\Widget\RecentlyViewed" uiComponent="widget_recently_viewed" page_size="'.$data['setting']['limit'].'" show_attributes="name,image,price" show_buttons="add_to_cart,add_to_compare,add_to_wishlist" template="product/widget/viewed/grid.phtml"}}</div>';
					}
					
					$data['block_content'] = $content;
                    
					$result['message'] = 'success';
					$sessionMessage = __('You saved the Recently Viewed Products block. Please wait to reload page.');
					break;
				...
				/* Deals Block */
				case "deals":
					$dataInit = ['use_slider', 'autoplay', 'stop_auto', 'navigation', 'pagination', 'use_tabs', 'loadmore', 'deals_template', 'deals_product_id'];
					$data = $this->reInitData($data, $dataInit);
					$categories = '';
					if(isset($data['setting']['category_id'])){
						$categories = implode(',',$data['setting']['category_id']);
					...
					if(isset($data['setting']['deals_product_id']) && $data['setting']['deals_template']==2){
						$template = 'products/deals/template-for-single-deal-product.phtml';
					}else{
						$template = 'products/deals/'.$data['setting']['template'];
					}
					
					
					$content = '{{block class="MGS\Mpanel\Block\Products\Deals" mgs_panel_title="'.$this->encodeHtml($data['setting']['title']).'" mgs_panel_note="'.$this->encodeHtml($data['setting']['additional_content']).'" limit="'.$data['setting']['limit'].'" perrow="'.$data['setting']['perrow'].'" deals_product_id="'.$data['setting']['deals_product_id'].'" ratio="'.$data['setting']['ratio'].'" category_ids="'.$categories.'" use_slider="'.$data['setting']['use_slider'].'" loadmore="'.$data['setting']['loadmore'].'" deals_template="'.$data['setting']['deals_template'].'" template="'.$template.'"';
					
					if($data['setting']['use_slider']){
						$content .= ' autoplay="'.$data['setting']['autoplay'].'" stop_auto="'.$data['setting']['stop_auto'].'" deals_product_id="'.$data['setting']['deals_product_id'].'" navigation="'.$data['setting']['navigation'].'" pagination="'.$data['setting']['pagination'].'" number_row="'.$data['setting']['number_row'].'"'.'" deals_template="'.$data['setting']['deals_template'].'"';
					}
				...
				case "tabs":
					if(isset($data['setting']['tabs']) && count($data['setting']['tabs'])>0){
						...
						$dataInit = ['use_slider', 'autoplay', 'stop_auto', 'navigation', 'pagination', 'use_tabs', 'loadmore', 'tab_template'];
						$data = $this->reInitData($data, $dataInit);
						$categories = '';
						if(isset($data['setting']['category_id'])){
							$categories = implode(',',$data['setting']['category_id']);
				...
				case "special_product":
					$dataInit = ['product_name', 'product_price', 'product_rating', 'product_description', 'product_addcart', 'prd_template'];
					$data = $this->reInitData($data, $dataInit);
					
					$content = '{{block class="MGS\Mpanel\Block\Products\SpecialProduct" mgs_panel_title="'.$this->encodeHtml($data['setting']['title']).'" mgs_panel_note="'.$this->encodeHtml($data['setting']['additional_content']).'" product_id="'.$data['setting']['product_id'].'" ratio="'.$data['setting']['ratio'].'" product_name="'.$data['setting']['product_name'].'" product_price="'.$data['setting']['product_price'].'" product_rating="'.$data['setting']['product_rating'].'" product_description="'.$data['setting']['product_description'].'"  prd_template="'.$data['setting']['prd_template'].'" product_addcart="'.$data['setting']['product_addcart'].'"';
				...
				/* Portfolio Block */
				case "portfolio":
					$dataInit = ['show_categories', 'show_thumbnail', 'use_slider', 'autoplay', 'stop_auto', 'navigation', 'pagination'];
					$data = $this->reInitData($data, $dataInit);
					$categories = implode(',',$data['setting']['categories']);
					$content = '{{block class="MGS\Portfolio\Block\Widget" mgs_panel_title="'.$this->encodeHtml($data['setting']['title']).'" mgs_panel_note="'.$this->encodeHtml($data['setting']['additional_content']).'" category_ids="'.$categories.'" limit="'.$data['setting']['limit'].'" perrow="'.$data['setting']['perrow'].'" show_categories="'.$data['setting']['show_categories'].'" show_thumbnail="'.$data['setting']['show_thumbnail'].'" use_slider="'.$data['setting']['use_slider'].'" template="widget/'.$data['setting']['template'].'"';
