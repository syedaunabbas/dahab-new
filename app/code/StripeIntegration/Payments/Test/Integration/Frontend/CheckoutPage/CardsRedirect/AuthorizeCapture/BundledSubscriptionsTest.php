...
class BundledSubscriptionsTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testDynamicBundleMixedTrialCart()
    {
        ...
        $event->triggerEvent('invoice.payment_succeeded', $invoiceId, $this);
        ...
        $event = $this->eventFactory->create();
        $event->triggerEvent("charge.succeeded", $subscription->latest_invoice->charge, $this);

        $event = $this->eventFactory->create();
        $event->triggerEvent("invoice.payment_succeeded", $subscription->latest_invoice->id, $this);
        ...
        $this->assertEquals("new", $newOrder->getState());
