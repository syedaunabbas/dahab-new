...
class PlaceOrderTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testMixedCart()
    {
        ...
        $event->triggerEvent("charge.succeeded", $invoice->charge, $this);
        ...
        $event->triggerEvent("invoice.payment_succeeded", $invoice, $this);
        ...
        $event->triggerEvent("charge.succeeded", $charge, $this);
    ...
    public function testMixedTrialCart()
    {
        ...
        $event = $this->eventFactory->create()->setType("charge.succeeded");
        $event->dispatch($subscription->latest_invoice->charge);
        $this->assertEquals(200, $event->getResponse()->getStatusCode());

        $event = $this->eventFactory->create()->setType("invoice.payment_succeeded");
        $event->dispatch($subscription->latest_invoice->id);
        $this->assertEquals(200, $event->getResponse()->getStatusCode());
